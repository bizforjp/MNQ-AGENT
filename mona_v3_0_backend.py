...
# -------------------------------------------------------------------
# TEMPORARY — Finding #3 investigation (Signal #9 phantom SL).
# Remove this route in the same commit that ships the root-cause fix.
# Do not leave deployed past the fix turn.
# -------------------------------------------------------------------
@app.get("/debug/signal/{signal_id}")
async def debug_signal(signal_id: int):
    try:
        with sqlite3.connect(DB_PATH) as conn:
            conn.execute("PRAGMA foreign_keys = ON")
            conn.row_factory = sqlite3.Row
            ...
            outcome_rows = conn.execute(
                "SELECT * FROM trade_outcomes WHERE signal_id=? "
                "ORDER BY outcome_id", (signal_id,),
            ).fetchall()
            eval_rows    = conn.execute(
                "SELECT * FROM evaluations WHERE signal_id=? "
                "ORDER BY eval_id", (signal_id,),
            ).fetchall()
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

    if sig_row is None and pos_row is None and not outcome_rows and not eval_rows:
        raise HTTPException(
            status_code=404,
            detail=f"signal_id={signal_id} not found in any v3 table",
        )

    # Derived helpers so the JSON is readable without an epoch converter.
    def _et(ms):
        if not ms:
            return None
    ...
    if pos_row is not None:
        opened = pos_row["opened_at_ms"]
        def _delta_min(ms):
            if not ms or not opened:
                return None
            return round((ms - opened) / 60_000, 2)
        derived = {
            "opened_at_et":                          _et(opened),
            "mae_bar_et":                            _et(pos_row["mae_bar_ms"]),
            "mae_offset_min_from_entry":             _delta_min(pos_row["mae_bar_ms"]),
            "mfe_bar_et":                            _et(pos_row["mfe_bar_ms"]),
            "mfe_offset_min_from_entry":             _delta_min(pos_row["mfe_bar_ms"]),
            "tp1_hit_bar_et":                        _et(pos_row["tp1_hit_bar_ms"]),
            "tp1_hit_offset_min_from_entry":         _delta_min(pos_row["tp1_hit_bar_ms"]),
            "last_heartbeat_bar_et":                 _et(pos_row["last_heartbeat_bar_ms"]),
            "last_heartbeat_offset_min_from_entry":  _delta_min(pos_row["last_heartbeat_bar_ms"]),
            "closed_at_et":                          _et(pos_row["closed_at_ms"]),
            "closed_offset_min_from_entry":          _delta_min(pos_row["closed_at_ms"]),
        }

    return {
        "signal_id":      signal_id,
        "signals_v3":     dict(sig_row) if sig_row else None,
        "positions":      dict(pos_row) if pos_row else None,
        "trade_outcomes": [dict(r) for r in outcome_rows],
        "evaluations":    [dict(r) for r in eval_rows],
        "derived":        derived,
        "_note": ("TEMPORARY — Finding #3 diagnostic. No heartbeats table "
                  "exists in the schema, so per-bar OHLC history is not "
                  "available; only the position's rolling state is. "
                  "Remove this route with the fix commit."),
    }
